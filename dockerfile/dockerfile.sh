...
# 어떤 특정 경로에 있는 Dockerfile or 특정 Dockerfile로 빌드하는 경우 -f 옵션
...
#추가된 파일까지 포함하려면 이미지를 다시 빌드해야 한다
docker build -t df2 .

#빌드, 컨테이너 실행, 내부명령어 bash 집
cd : 폴더이동
ls : 파일목록 확인
...
# ENTRYPOINT: 컨테이너 시작했을 때 실행할 명령어 설정
...
# ENTRYPOINT 이미지 생성 후, 컨테이너 실행할 때! 실행
...
#Dockerfile에 ENV 설정이 되어있어서. 별로도 -e PW 세팅을 안 하고 실행해도 된다
docker ps -a 
#비밀번호 같은 중요한 정보는 ENV로 설정을 고정해두는 것보다, 실행 시점에 -e 옵션으로 지정하는 것을 권장
