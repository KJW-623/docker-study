...
#local pc에 다른 oracle db가 설치되어 있지 않으면?
#-p 1521:1521
# 생성시 1522 포트 사용이유는? 현재 pc에 설치된 OracleDB 서버가 1521 포트를 사용중

#볼륨 Bind Mount 방식
# -v d:/study/docker-study/sample/oracledb/oradata:/pot/oracle/oradata
# -v //d/study/docker-study/sample/oracledb/oradata:/pot/oracle/oradata

#오라클 DB 컨테이너 생성 후, 계정 생성 및 접근 권할 설정
#오라클 컨테이너 서버 접속 (명령어로 내부 접근)
docker exec -it oracle19c bash

#db 접근 (컨테이너 내부 bash에서 명령어)
sqlplus / as sysdba
...
#DB 정장접속 여부 확인 (sqldveloper 등 기타 SQL Tool)
사용자이름 : scott
비번 : tiger
호스트이름 : localhost
포트번호 : 1522
