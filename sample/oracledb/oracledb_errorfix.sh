...
#제대로 실행이 안 된 상태
Connented ...... idle instance.
ORA-01034: ORACLE not avaoilable

#오라클 인스턴스 실행
startup
#인스턴스 실행을 시도하였으나, 파일이 없어서 실행이 안 됨.
#cloud not open parameter file '/opt/oracle/product/19c/dbhome_1/dbs/initorcl.ora'
...
cp /opt/oracle/admin/ORCL/pfile/init.ora /product/19c/dbhome_1/dbs/initorcl.ora
...
#접속시 해당 서비스 이름의 리스너가 없다. 리스너 인식 오류 케이스
