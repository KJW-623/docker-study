...
# docker run으로 실행시, 이미지가 없으면 알아서 다운로드
...
# 이미지 삭제하기
docker image rm 이미지이름 or 이미지ID
docker rmi 이미지이름 or 이미지ID
...
# 컨테이너 종료
docker stop 컨테이너 이름 or 컨테이너ID #윈도우 >> 시작 >> 전원 >> 시스템종료
docker kill 컨테이너 이름 or 컨테이너ID #강제 종료

# 중지된 컨테이너 실행
docker start 컨테이너 이름 or 컨테이너ID
