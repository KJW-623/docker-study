...
docker run -d --name busybox sleep infinite
# 테스트용 작은 리눅스
# sleep 무한으로 돌아라~ 라는 프로세스가 수행중이기 때문에, 컨테이너가 종료되지 않고 유지
...
# busybox라는 이름의 컨테이너에게 sh(커맨드창) 명령어를 수행하도록 시키겠다
# -it 실시간 대화형 (현재 터미널 창에서 연결된 상태로 사용)

# 접속된 상태에서는 리눅스 명령어로 수행 확인
ls  #현재 경로에 존재하는 파일 목록
